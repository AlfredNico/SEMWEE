...
export class ResizableDirective {
  ...
  @Input() isDoubleclicedBtn: boolean = false;
  @Output() isDoubleclicedBtnEvent = new EventEmitter<boolean>();
  ...
  @HostListener('dblclick', ['$event']) onLeave(e: MouseEvent) {
    ...
    if (this.isDbClicked == true && this.isDoubleclicedBtn === true) {
      const tableCells = Array.from(
        this.table.querySelectorAll('.mat-row')
      ).map((row: any) => row.querySelectorAll('.mat-cell').item(this.index));
      ...
      const chiled = div.childNodes[1] as HTMLElement;
      chiled.style.width = `${this.maxWidth}px`;

      let formSearch = this.column.childNodes[1] as HTMLElement;
      const chi = formSearch.childNodes[0] as HTMLElement;
      chi.style.width = `${this.maxWidth}px`;

      if (this.maxWidth > 30) formSearch.style.width = `${this.maxWidth}px`;
      ...
      const chiled = div.childNodes[1] as HTMLElement;
      chiled.style.width = `${this.minWidth}px`;

      let formSearch = this.column.childNodes[1] as HTMLElement;
      const chi = formSearch.childNodes[0] as HTMLElement;
      chi.style.width = `${this.minWidth}px`;

      if (this.minWidth > 30) formSearch.style.width = `${this.minWidth}px`;
    ...
    this.isDoubleclicedBtn = false;
    this.isDoubleclicedBtnEvent.emit(false);

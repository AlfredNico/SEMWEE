...
@Component({
  ...
  styles: [`
    .mat-form-field-appearance-outline .mat-form-field-infix {
        padding: .5em 0 0.7em 0 !important;
    }
...
export class EditComponent implements OnInit {
  ...
  constructor(private fb: FormBuilder, @Inject(MAT_DIALOG_DATA) public data: Projects, public dialogRef: MatDialogRef<EditComponent>, private projetctService: ProjectsService, private notifs: NotificationService, private custumValidator: CustomValidationService, private common: CommonService) {
  ...
  ngOnInit(): void {
    this.filteredCounrty = this.form.get("country_project").valueChanges
      .pipe(
        startWith(''),
        map(value => {
          return this.countries.filter(x => x.name.toLowerCase().includes(value.toLowerCase()));
        })
      )

    this.form.get("language_project").valueChanges
      .pipe(
        map(value => {
          this.form.patchValue({
            'path_project': `/${value}/*`
  ...
  async onSubmit() {
    ...
    } else if (this.imageLandscape != undefined || this.imageSquared != undefined) {
    ...
    if (this.imageLandscape instanceof File || this.imageSquared instanceof File) {
      if (this.form.valid) {
      ...
      reader.onload = () => {
        ...
        img.onload = () => {
          ...
          if (width > height) {
            ...
            this.notifs.warn(`this is not landscape image:  ${width} * ${height}`);
      ...
      reader.onload = () => {
        ...
        img.onload = () => {
          ...
          if (width == height) {
            ...
            this.notifs.warn(`this is not squared image:  ${width} * ${height}`);
